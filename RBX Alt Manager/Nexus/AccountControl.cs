...
namespace RBX_Alt_Manager.Forms
{
    public partial class AccountControl : Form
    {
        ...
        public ConcurrentDictionary<WebSocketContext, ControlledAccount> ContextList = new ConcurrentDictionary<WebSocketContext, ControlledAccount>();
        ...
        private void OpenServer()
        {
            ...
            // is an application JSON "ping" text frame, not a protocol Pong, and many executor socket
            // shims never auto-Pong — so the sweep drops healthy sessions at random. We have our own
            // liveness (LastPing + AutoRelaunch), so disable the sweep to stop the false disconnects.
