        ...
        public void Run()
        {
            ThreadPool.QueueUserWorkItem((o) =>
            {
                while (_listener.IsListening)
                {
                    ...
                    // A transient GetContext() throw used to fall out of the loop and silently kill the
                    // whole web API with no log. Isolate the accept so one bad accept doesn't stop serving.
                    try { context = _listener.GetContext(); }
                    catch (Exception ax)
                    {
                        if (!_listener.IsListening) break; // Stop() was called — normal shutdown
                        Program.Logger.Error($"WebServer accept error: {ax.Message}");
                        continue;
                    }
