...
namespace RBX_Alt_Manager
{
    static class Program
    {
        ...
        // The Chromium runtime (libcef.dll + GPU libs) can live in a "bin" subfolder to keep the app
        // folder tidy. Add it to the native DLL search path so those loads resolve; fall back to the
        // exe folder for the flat/dev layout. Returns the folder CEF resources actually live in.
        public static string CefDirectory
        {
            get
            {
                string baseDir = AppDomain.CurrentDomain.BaseDirectory;
                string bin = Path.Combine(baseDir, "bin");
                return Directory.Exists(bin) && File.Exists(Path.Combine(bin, "libcef.dll")) ? bin : baseDir;
            }
        }
        ...
        static void Main(params string[] Arguments)
        {
            ...
            try
            {
                ...
                // MessageBox.Show("Some features may not work properly if you ran the account manager as admin!", "KingsRAM", MessageBoxButtons.OK, MessageBoxIcon.Error); // I don't think this is an issue anymore
            }
            catch { }
            ...
            if (!IsAutoUpdater && !File.Exists($"{Application.ExecutablePath}.config"))
            {
                ...
                {
                    if (!Utilities.YesNoPrompt("KingsRAM", "It is recommended you install KingsRAM to it's own folder", "Skip this check and install here anyways?", false))
                        Environment.Exit(4);
                }
            }
            ...
            using (var ndpKey = RegistryKey.OpenBaseKey(RegistryHive.LocalMachine, RegistryView.Registry32)?.OpenSubKey(subkey))
            {
                ...
                {
                    if (MessageBox.Show("Failed to detect .NET Framework 4.7.2, would you like to install it now?\n(Required in order to use the account manager)\nIn case this error persists, fully re-install the account manager.", "KingsRAM", MessageBoxButtons.YesNo, MessageBoxIcon.Warning) == DialogResult.Yes)
            ...
            if (mutex.WaitOne(TimeSpan.Zero, true))
            {
                ...
                MessageBox.Show("KingsRAM is already running!", "KingsRAM", MessageBoxButtons.OK, MessageBoxIcon.Error);
#endif
        }
    }
}