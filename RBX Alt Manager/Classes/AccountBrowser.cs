...
namespace RBX_Alt_Manager.Classes
{
    ...
    internal class CefBrowser : Form
    {
        ...
        // Initialize CefSharp on first use, loading the Chromium runtime from wherever it actually lives
        // (the "bin" subfolder in the tidied release layout, or the exe folder in the flat/dev layout).
        private static void EnsureCefInitialized()
        {
            if (Cef.IsInitialized) return;

            string CefDir = Program.CefDirectory;

            if (!File.Exists(Path.Combine(CefDir, "libcef.dll")))
                throw new FileNotFoundException("Unable to locate the Chromium runtime (libcef.dll). Re-extract KingsRAM without moving any files.");
