...
namespace RBX_Alt_Manager.Classes
{
    internal class Batch
    {
        ...
        private static readonly object PendingBatchLock = new object();
        private static readonly object PendingPlaceLock = new object();

        private static readonly ConcurrentDictionary<long, long> PlaceUniversePair = new ConcurrentDictionary<long, long>();
        public static readonly ConcurrentDictionary<long, GameDetails> PlaceDetails = new ConcurrentDictionary<long, GameDetails>();
        ...
        private static async Task<JArray> DoBatchRequest()
        {
            ...
            while (PendingBatch.Count > 0)
            {
                ...
                lock (PendingBatchLock)
                {
                    int Take = Math.Min(PendingBatch.Count, 100);
                    Pending.AddRange(PendingBatch.GetRange(0, Take));
                    PendingBatch.RemoveRange(0, Take);
                }
        ...
        private static async Task DoPlaceRequest()
        {
            ...
            {
                if (waited >= 10000) return; // no logged-in account after ~10s; give up so awaiting callers don't hang forever
                await Task.Delay(80);
                waited += 80;
            }
            ...
            while (PendingPlace.Count > 0)
            {
                ...
                lock (PendingPlaceLock)
                {
                    int Take = Math.Min(PendingPlace.Count, 50);
                    Pending.AddRange(PendingPlace.GetRange(0, Take));
                    PendingPlace.RemoveRange(0, Take);
                }
                ...
                if (DetailsResponse.IsSuccessful)
                {
                    ...
                    foreach (GameDetails game in Games)
                    {
                        ...
                        PlaceDetails.TryAdd(PlaceId, game);

                        PlaceUniversePair.TryAdd(PlaceId, game.universeId); // game["universeId"].Value<long
                    }
                }
            }
        }
