...
namespace RBX_Alt_Manager
{
    public class Account : IComparable<Account>
    {
        ...
        // Serializes launches for THIS account. Manual launch, batch, the web API and Nexus AutoRelaunch
        // can all call JoinServer concurrently; two overlapping launches double-login the account and
        // Roblox kicks the first session ("played from another device"). Try-acquire, reject if busy.
        ...
        public Account(string Cookie, string AccountJSON = null)
        {
            ...
            // Parse the external Roblox payload tolerantly: a strict typed parse (MissingMemberHandling.Error)
            // would reject every account the moment Roblox adds a field to my/account/json.
            if (!string.IsNullOrEmpty(AccountJSON) && AccountJSON.TryParseJson(out JObject Data) && Data["Name"] != null && Data["UserId"] != null)
            {
        ...
        public async Task<string> JoinServer(long PlaceID, string JobID = "", bool FollowUser = false, bool JoinVIP = false, bool Internal = false) // oh god i am not refactoring everything to be async im sorry
        {
            ...
            LastUse = DateTime.Now; // a launched account is, by definition, in use — keeps AutoCookieRefresh's
                                    // 20-day-idle signout from kicking a client we just launched.
            ...
            if (GetAuthTicket(out string Ticket))
            {
                ...
                if (JoinVIP)
                {
                    ...
                    if (result.IsSuccessful && !result.Content.Contains("\"AllUsers\""))
                    {
                        ...
                        {
                            if (Utilities.YesNoPrompt("KingsRAM", "Account Manager has detected your account's privacy settings do not allow you to join private servers.", "Would you like to change this setting to Everyone now?"))
                            {
