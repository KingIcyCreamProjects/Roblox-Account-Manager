    ...
    public class BorderedRichTextBox : RichTextBox // https://github.com/r-aghaei/TextBoxBorderColor/blob/master/src/TextBoxBorderColor/MyTextBox.cs
    {
        ...
        public Color BorderColor
        {
            ...
            set
            {
                ...
                opaquePen?.Dispose();
                borderPen?.Dispose();
                opaquePen = new Pen(Color.FromArgb(255, value.R, value.G, value.B)); // Pen-from-Color; no throwaway Brush to leak
        ...
        protected override void Dispose(bool disposing)
        {
            if (disposing)
            {
                opaquePen?.Dispose();
                borderPen?.Dispose();
                opaquePen = null; // null out so a late WM_NCPAINT hits the WndProc guard and recreates,
                borderPen = null; // rather than drawing with a disposed Pen (ArgumentException)
            }
