...
namespace Auto_Update
{
    public partial class AutoUpdater : Form
    {
        ...
        private async Task Download()
        {
            ...
            string Releases = await client.GetStringAsync("https://api.github.com/repos/KingIcyCreamProjects/Roblox-Account-Manager/releases/latest");

            // Pin to the expected asset name instead of grabbing the first browser_download_url in the JSON
            // (a stray/extra asset could otherwise be downloaded and run).
            var Urls = Regex.Matches(Releases, @"""browser_download_url"":\s*""([^""]+)""").Cast<Match>().Select(m => m.Groups[1].Value).ToArray();

            string DownloadUrl = Urls.FirstOrDefault(u => u.EndsWith("/RobloxAccountManager.zip", StringComparison.OrdinalIgnoreCase));
            string ChecksumUrl = Urls.FirstOrDefault(u => u.EndsWith("/RobloxAccountManager.zip.sha256", StringComparison.OrdinalIgnoreCase));

            if (string.IsNullOrEmpty(DownloadUrl)) // fall back to the old first-asset behavior for older releases
                DownloadUrl = Urls.FirstOrDefault(u => !u.EndsWith(".sha256", StringComparison.OrdinalIgnoreCase));
            ...
            if (!string.IsNullOrEmpty(DownloadUrl))
            {
                ...
                // Integrity: verify the download's SHA-256 against the release's checksum sidecar before
                // extracting and running it. Protects against corrupted/truncated downloads and network
                // tampering. (A checksum from the same release can't defend a compromised GitHub account —
                // that needs release signing; see the project notes.)
                if (!string.IsNullOrEmpty(ChecksumUrl))
                {
                    SetStatus("Verifying download...");

                    string Expected = (await client.GetStringAsync(ChecksumUrl)).Trim().Split(' ', '\t', '\n', '\r')[0];
                    string Actual;
                    ...
                    if (!string.Equals(Expected, Actual, StringComparison.OrdinalIgnoreCase))
                    {
                        ...
                        Invoke(new Action(() =>
                        {
                            MessageBox.Show(this, $"Update integrity check FAILED.\n\nThe download does not match the published checksum and will NOT be installed.\n\nExpected: {Expected}\nActual:   {Actual}", "KingsRAM Updater", MessageBoxButtons.OK, MessageBoxIcon.Error);
                            Environment.Exit(0);
                        }));
        ...
        private void Extract()
        {
            ...
            {
                if (Utilities.YesNoPrompt("KingsRAM", "Create Start-Menu shortcut?", "", false))
                {
                    string StartMenuPath = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.CommonStartMenu), "Programs", "KingsRAM");

                    if (!Directory.Exists(StartMenuPath))
                        Directory.CreateDirectory(StartMenuPath);

                    IWshShortcut shortcut = (IWshShortcut)new WshShell().CreateShortcut(Path.Combine(StartMenuPath, "KingsRAM.lnk"));

                    shortcut.Description = "KingsRAM";
