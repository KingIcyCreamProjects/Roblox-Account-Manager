...
namespace RBX_Alt_Manager.Forms
{
    public partial class ThemeEditor : Form
    {
        ...
        private static Color TryHtmlColor(string html, Color fallback)
        {
            try { return ColorTranslator.FromHtml(html); }
            catch { return fallback; } // a malformed color string in RAMTheme.ini must not crash startup
        }
        ...
        public static void LoadTheme()
        {
            ...
            if (Theme.Exists("AccountsBG")) AccountBackground = TryHtmlColor(Theme.Get("AccountsBG"), AccountBackground);
            if (Theme.Exists("AccountsFG")) AccountForeground = TryHtmlColor(Theme.Get("AccountsFG"), AccountForeground);

            if (Theme.Exists("ButtonsBG")) ButtonsBackground = TryHtmlColor(Theme.Get("ButtonsBG"), ButtonsBackground);
            if (Theme.Exists("ButtonsFG")) ButtonsForeground = TryHtmlColor(Theme.Get("ButtonsFG"), ButtonsForeground);
            if (Theme.Exists("ButtonsBC")) ButtonsBorder = TryHtmlColor(Theme.Get("ButtonsBC"), ButtonsBorder);
            if (Theme.Exists("ButtonStyle") && Enum.TryParse(Theme.Get("ButtonStyle"), out FlatStyle BS)) ButtonStyle = BS;

            if (Theme.Exists("FormsBG")) FormsBackground = TryHtmlColor(Theme.Get("FormsBG"), FormsBackground);
            if (Theme.Exists("FormsFG")) FormsForeground = TryHtmlColor(Theme.Get("FormsFG"), FormsForeground);
            ...
            if (Theme.Exists("TextBoxesBG")) TextBoxesBackground = TryHtmlColor(Theme.Get("TextBoxesBG"), TextBoxesBackground);
            if (Theme.Exists("TextBoxesFG")) TextBoxesForeground = TryHtmlColor(Theme.Get("TextBoxesFG"), TextBoxesForeground);
            if (Theme.Exists("TextBoxesBC")) TextBoxesBorder = TryHtmlColor(Theme.Get("TextBoxesBC"), TextBoxesBorder);

            if (Theme.Exists("TextBoxesBG") && !Theme.Exists("LabelsTransparent")) LabelTransparent = false; // support old themes
            if (Theme.Exists("LabelsBC")) LabelBackground = TryHtmlColor(Theme.Get("LabelsBC"), TextBoxesBackground); else LabelBackground = TextBoxesBackground;
            if (Theme.Exists("LabelsFC")) LabelForeground = TryHtmlColor(Theme.Get("LabelsFC"), TextBoxesForeground); else LabelForeground = TextBoxesForeground;
