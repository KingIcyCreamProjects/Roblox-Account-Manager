![KingsRAM](Images/KingsRAM-banner.png)

[![Download](https://img.shields.io/github/v/release/KingIcyCreamProjects/Roblox-Account-Manager)](https://github.com/KingIcyCreamProjects/Roblox-Account-Manager/releases/latest)
[![Latest Downloads](https://img.shields.io/github/downloads/KingIcyCreamProjects/Roblox-Account-Manager/latest/total)](https://github.com/KingIcyCreamProjects/Roblox-Account-Manager/releases)
![License](https://img.shields.io/github/license/KingIcyCreamProjects/Roblox-Account-Manager)

<!-- BEGIN LATEST DOWNLOAD BUTTON -->
[![Download zip](https://custom-icon-badges.herokuapp.com/badge/-Download-blue?style=for-the-badge&logo=download&logoColor=white "Download")](https://github.com/KingIcyCreamProjects/Roblox-Account-Manager/releases/latest/download/RobloxAccountManager.zip)
<!-- END LATEST DOWNLOAD BUTTON -->

> [!IMPORTANT]
> ### ⚠️ Nothing happens when you run it? You must UNBLOCK the download first.
> Windows **SmartScreen** silently blocks unsigned apps downloaded from the internet, so a double-click often does nothing. This app is open-source and unsigned — it is **not** a virus (antivirus false positives are normal for account managers; see the [FAQ](#q-why-is-this-program-detected-as-a-virus)). Do **one** of these:
> - **Best — before extracting:** right-click **`RobloxAccountManager.zip`** → **Properties** → tick **Unblock** (bottom right) → **OK**, then extract.
> - **Already extracted?** Open PowerShell in the app folder and run: `Get-ChildItem -Recurse | Unblock-File`
> - If you see a blue **"Windows protected your PC"** box: click **More info → Run anyway**.
> - If your antivirus deletes files, add an **exclusion** for the app folder.

# KingsRAM
**KingsRAM** is a personal, security-hardened fork of [Roblox Account Manager](https://github.com/ic3w0lf22/Roblox-Account-Manager) by ic3w0lf22 (GNU GPLv3), maintained by [KingIcyCreamProjects](https://github.com/KingIcyCreamProjects). It keeps the original's full feature set and adds a brand-new interface, reliability fixes, and a security hardening pass (see [What's new](#whats-new)).

Add multiple accounts into one application and easily play on alt accounts without switching accounts. Useful for games that require grinding off other players, storage accounts that hold in-game items or currency, or just keeping many accounts organized and ready to use.

Multiple Roblox instances are built in but [must be manually enabled](#q-how-do-i-enable-multi-roblox).

[Jump to the full feature list.](#features)
...
- Runs in an embedded Microsoft Edge WebView2 window. Prefer the classic look? **Appearance → "Use classic UI"**, and switch back anytime via the classic **Settings → "Use Modern UI"**.

### v1.0 — Reliability & security hardening
**Fewer random disconnects**
- The Account Control (Nexus) websocket no longer drops healthy sessions — the keep-alive sweep that force-closed clients whose socket didn't answer within one second has been disabled in favor of the app's own liveness check.
- Fixed a reconnect/teleport race that could show a still-connected account as offline.
- Overlapping launches of the same account are now blocked, so Roblox no longer kicks your first session ("played from another device").
- Accounts you just launched are no longer signed out by the idle cookie refresh.

**Reliability**
- Crash-proof (atomic) account saves — a crash or power loss mid-save can no longer corrupt `AccountData.json`.
- The server list no longer hammers the Roblox API when it gets rate-limited.
- Fixed a crash when a missing asset fails to load, GDI handle leaks in the custom text controls, and a case where the local web API could silently stop serving.

**Security & privacy**
- The auto-updater verifies each download's SHA-256 checksum against a published `.sha256` before installing.
- The local web API denies by default (a 6+ character password is required for sensitive calls) and rejects cross-origin / DNS-rebinding requests.
- Account cookies are encrypted per Windows user, and the released build no longer embeds any local build paths.
...
# Download
To install, download the latest [Release](https://github.com/KingIcyCreamProjects/Roblox-Account-Manager/releases/latest) (`RobloxAccountManager.zip`), extract the whole folder somewhere (e.g. your Desktop), and run **Roblox Account Manager.exe** from inside it. Keep all the files together.

> **Nothing happens when you run it?** You need to unblock the download — see the [**unblock instructions at the top of this page**](#-nothing-happens-when-you-run-it-you-must-unblock-the-download-first). This is a Windows SmartScreen block on unsigned apps, not a real problem with KingsRAM.

If it still won't start, make sure the [Latest .NET Framework](https://dotnet.microsoft.com/download/dotnet-framework) is installed.
...
| [Multi Roblox](https://github.com/KingIcyCreamProjects/Roblox-Account-Manager/blob/master/README.md#q-how-do-i-enable-multi-roblox) | [DISABLED BY DEFAULT, READ FAQ] RAM comes with a built-in multi Roblox allowing multiple Roblox clients to be open at once | **If this doesn't work for you, make sure no Roblox processes are running in the background by checking in task manager, then restart RAM** |
...
| Account Control | Control your in-game accounts using the `Account Control` window in RAM | Click `Account Control` in the main window - [Documentation](https://github.com/KingIcyCreamProjects/Roblox-Account-Manager/blob/master/RBX%20Alt%20Manager/Nexus/NexusDocs.md) |
...
# Preview
![KingsRAM modern UI](Images/KingsRAM-modern-ui.png)
